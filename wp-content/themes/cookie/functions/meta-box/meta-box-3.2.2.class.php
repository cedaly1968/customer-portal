...
class RW_Meta_Box {
	...
	function insert_images() {
		...
		foreach ($_POST['attachments'] as $attachment_id => $attachment) {
			...
			$li .= "<a title='" . __('Delete this image','meydjer') . "' class='rw-delete-file' href='#' rel='$nonce|$post_id|$id|$attachment_id'>" . __('Delete','meydjer') . "</a>";
	...
	function show_field_file($field, $meta) {
		...
		if (!empty($meta)) {
			...
			foreach ($meta as $att) {
				...
				echo "<li>" . wp_get_attachment_link($att, '' , false, false, ' ') . " (<a class='rw-delete-file' href='#' rel='$nonce|{$post->ID}|{$field['id']}|$att'>" . __('Delete','meydjer') . "</a>)</li>";
			}
			echo '</ol>';
		}
		...
		echo "<div style='clear: both'><strong>" . __('Upload new files','meydjer') . "</strong></div>
			<div class='new-files'>
				...
				<a class='rw-add-file' href='#'>" . __('Add more file','meydjer') . "</a>
			</div>
		</td>";
	}
	...
	function show_field_image($field, $meta) {
		...
		if (!empty($meta)) {
			...
			foreach ($images as $image) {
				...
				echo "<li id='item_$image'>
						<img src='$src' />
						<a title='" . __('Delete this image','meydjer') . "' class='rw-delete-file' href='#' rel='$nonce_delete|{$post->ID}|{$field['id']}|$image'>" . __('Delete','meydjer') . "</a>
		...
		echo "<a href='#' class='rw-upload-button button' rel='{$post->ID}|{$field['id']}'>" . __('Add more images','meydjer') . "</a>";
	}
	...
	function show_field_color($field, $meta) {
		...
		echo "<input class='rw-color' type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' />
			  <a href='#' class='rw-color-select' rel='{$field['id']}'>" . __('Select a color','meydjer') . "</a>

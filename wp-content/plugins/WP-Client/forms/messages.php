...
if ( isset( $_POST['comment'] ) && '' != $_POST['comment'] ) {
    ...
    if ( isset( $_POST['from_messages_page'] ) && 1 == $_POST['from_messages_page'] ) {
        $sent_to_row = $_POST['clients'];
        if( $sent_to_row == 'all' ) {
            $sent_to = $this->get_client_ids();
        } else {
            $sent_to = explode( ',', $sent_to_row );
        }
    ...
    if ( isset( $_POST['from_messages_page'] ) && 1 == $_POST['from_messages_page'] && is_array( $sent_to ) ) {
        foreach( $sent_to as $val ) {
    ...
    if ( get_option( 'wpc_notify_message2' ) == 'yes' ) {
        ...
        if ( isset( $_POST['from_messages_page'] ) && 1 == $_POST['from_messages_page'] && is_array( $sent_to ) ) {
            foreach( $sent_to as $val ) {
                ...
                $send_to_email  = get_userdata( $val )->get( 'user_email' );
                ...
                $args = array( 'client_id' => $val, 'user_name' => $username, 'message' => nl2br( htmlspecialchars( stripslashes( $_POST['comment'] ) ) )  );
                $subject = $this->replace_placeholders( $wpc_templates['emails']['notify_client_about_message']['subject'], $args, 'notify_client_about_message' );
                $subject = htmlentities( $subject, ENT_QUOTES, 'UTF-8' );
                $message = $this->replace_placeholders( $wpc_templates['emails']['notify_client_about_message']['body'], $args, 'notify_client_about_message' );
            ...
            $args = array( 'client_id' => $sent_to, 'user_name' => $username, 'message' => nl2br( htmlspecialchars( stripslashes( $_POST['comment'] ) ) )  );
            $subject = $this->replace_placeholders( $wpc_templates['emails']['notify_client_about_message']['subject'], $args, 'notify_client_about_message' );
            $subject = htmlentities( $subject, ENT_QUOTES, 'UTF-8' );
            $message = $this->replace_placeholders( $wpc_templates['emails']['notify_client_about_message']['body'], $args, 'notify_client_about_message' );
...
<?php if ( 'all_messages' == $mode ): ?>
    ...
    <table>
        <tr>
            <td>
                <label for="clients"><?php _e( 'Sent message to', WPC_CLIENT_TEXT_DOMAIN ) ?>: </label>

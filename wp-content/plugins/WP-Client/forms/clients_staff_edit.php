...
if ( isset( $_REQUEST['update_user'] ) ) {
    ...
    if ( empty( $error ) ) {
        ...
        $userdata = array(
            ...
            'parent_client_id'  => esc_attr( $_REQUEST['clients'] ),
        );
        if ( isset( $_REQUEST['user_data']['ID'] ) ) {
        ...
        if ( !isset( $userdata['ID'] ) ) {
            ...
            if ( 'on' == $userdata['send_password'] || '1' == $userdata['send_password'] ) {
                ...
                $subject = $this->replace_placeholders( $wpc_templates['emails']['staff_created']['subject'], $args, 'staff_created' );
                $subject = htmlentities( $subject, ENT_QUOTES, 'UTF-8' );
                $message = $this->replace_placeholders( $wpc_templates['emails']['staff_created']['body'], $args, 'staff_created' );
            ...
            <form name="edit_employee" id="edit_employee" method="post" >
                ...
                <table class="form-table">
                    <tbody>
                        ...
                        <tr>
                            <td colspan="2">
                                <span class="edit"><a href="#popup_block2" rel="clients" class="fancybox_link radio" title="assign clients" ><?php _e( 'Assign to Client:', WPC_CLIENT_TEXT_DOMAIN ) ?></a></span>&nbsp;&nbsp;&nbsp;<span class="edit" id="counter_clients">(  )</span>
                                <input type="hidden" name="clients" id="clients" value="<?php echo ( isset($user_data['parent_client_id']) ? $user_data['parent_client_id'] : '' ); ?>" />
                            </td>
                        </tr>
                        <?php endif; ?>
                    </tbody>

...
if (($REQUEST_METHOD == 'POST') && ($mode == 'product_modify')) {
    ...
    // If no errors
		...
		if (!empty($active_modules['HighCharts'])) {
                         array_unshift($highchartids, "0");
                         array_push($highchartids, "0");    
			foreach ($highchartids as $hch => $hchart) {
				$hchartid[] = $hchart;
				 }
 				$highchartid = implode(',',$hchartid);
				$hquery_data = array(
				    'highchart'   => $highchartid
				 );
		    $query_data=array_merge($query_data,$hquery_data);
       }
...
$smarty->assign('allcategories', func_data_cache_get("get_categories_tree", array(0, true, $shop_language, $user_account['membershipid'])));
 if (
        !empty($active_modules['HighCharts'])
    ) 
{

$smarty->assign('highcharts',func_query("SELECT id,chart_title FROM  `hc_chart` WHERE  `avail` =  'Y'"));

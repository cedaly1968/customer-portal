...
<tr>
  {if $geid ne ''}<td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[highchartids]" /></td>{/if}
  <td class="FormButton" nowrap="nowrap">Highchart:</td>
  <td class="ProductDetails">
 
  <select name="highchartids[]" class="InputWidth" multiple="multiple" size="8">
{foreach from=$highcharts item=pchart key=ph}
    <option value="{$pchart.id}" {if in_array($pchart.id, $product.highchart)} selected="selected"{/if}>{$pchart.chart_title}</option>
{/foreach}
  </select>
  </td>
</tr>

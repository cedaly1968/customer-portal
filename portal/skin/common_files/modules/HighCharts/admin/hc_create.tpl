...
{if $mode1 eq "update_chart"}
 <input type="hidden" name="mode1" value="update_chart"/>
 <input type="hidden" name="chart_edit_id" value="{$chart_edit_id}"/>
{/if}
 <input type="hidden" name="mode" value="update"/>
	...
	<table class="chart_table" cellpadding="0" cellspacing="0" width="100%">
		...
		<tr>
			...
			<td>
		       <select name="posted_values[Table]" id="table" onchange="load_table(this.value)">
		           ...
		           <option value="{$st}" {if $post1.chart_Table eq $st} selected="selected" {/if}>{$st}</option>
		           {/if}
		           {/foreach}
		         </select>
		    </td>
		...
		<tr>
			...
			<td> Label: <input type="text" name="posted_values[X-Axis][Label]" value="{$post1.X-Axis_label|escape:"html"}" />    </td>
			<td>Y-Axis  <select name="posted_values[Y-Axis][Y]" id="ytable">
		           <option value=""></option>
		         </select></td>
			<td>  Label: <input type="text" name="posted_values[Y-Axis][Label]" value="{$post1.Y-Axis_label|escape:"html"}"/>    </td>
		</tr>
		...
		<tr>
			...
			<td>
		         <select name="posted_values[PlotType]" id="plottype" onchange="load_options(this.value)">
		           ...
		           <option value="{$gv_pt.plot_id},{$gv_pt.attribute}" {if $post1.plot_type1 eq $gv_pt.attribute} selected="selected" {/if}>{$gv_pt.attribute}</option>
		           {/foreach}
		         </select>
			</td>
		</tr>
...
<div style="text-align:center;"  id="loading">loading{*<img src="/skin/common_files/css/images/loading.gif" />*} </div>
...
$(".slidingDiv").hide();
$("#loading").hide();
load_table("{$post1.chart_Table}");
load_options("{$post1.plot_type}");
{rdelim});
{literal}

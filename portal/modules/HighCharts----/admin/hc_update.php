...
foreach ( $posted_values ['chart_options'] as $kval => $cval ) {  
	  if (count($cval) != 1 && $kval=='series') {
		$brackts = explode ( ',', $cval ['bracket'] );
		//$series1 .= $kval . ': ' . $brackts ['0']; //// root paranthesis start
		foreach ( $cval as $cv => $cv1 ) {
			...
			if (is_array ( $cv1 )) {
				$series1 .= insertarray ( $cv, $cv1 );
			}
		}
		//$series1 .= $brackts ['1'] . ','; //// root paranthesis ends
	}
	
	
	  if (count($cval) != 1 &&  $kval=='xAxis') {
		$brackts = explode ( ',', $cval ['bracket'] );
		//$xAxis1 .= $kval . ': ' . $brackts ['0']; //// root paranthesis start
		foreach ( $cval as $cv => $cv1 ) {
			$a1 = array_keys ( $cval );
			$a = end ( $a1 );
			...
			if (is_array ( $cv1 )) {
				$xAxis1 .= insertarray ( $cv, $cv1 );
			}
		}
		//$xAxis1 .= $brackts ['1'] . ','; //// root paranthesis ends
	}
	
	
	  if (count($cval) != 1 && $kval=='yAxis') {
		$brackts = explode ( ',', $cval ['bracket'] );
		//$yAxis1 .= $kval . ': ' . $brackts ['0']; //// root paranthesis start
		foreach ( $cval as $cv => $cv1 ) {
			$a1 = array_keys ( $cval );
			$a = end ( $a1 );
			...
			if (is_array ( $cv1 )) {
				$yAxis1 .= insertarray ( $cv, $cv1 );
			}
		}
		//$yAxis1 .= $brackts ['1'] . ','; //// root paranthesis ends
	}
	
	  if (count($cval) != 1 && $kval!='series' && $kval!='xAxis' && $kval!='yAxis') {
		$brackts = explode ( ',', $cval ['bracket'] );
...
$plotyp = $ptype[1];
 ...
 $series1 = mysql_real_escape_string($series1);
 $xAxis1 = mysql_real_escape_string($xAxis1);
 $yAxis1 = mysql_real_escape_string($yAxis1);


$query_data = array ('chart_title' => $posted_values ['chart_title'], 'chart_sub_title' => $posted_values ['chart_sub_title'], 'chart_Table' => $posted_values ['Table'], 'X-Axis_field' => $posted_values ['X-Axis'] ['X'], 'X-Axis_label' => $posted_values ['X-Axis'] ['Label'], 'Y-Axis_field' => $posted_values ['Y-Axis'] ['Y'], 'Y-Axis_label' => $posted_values ['Y-Axis'] ['Label'], 'series' => implode(",", $posted_values['Series']), 'plot_type' => $plotyp, 'chart_setting' => $insertarray ,'series_setting' => $series1 ,'xAxis__setting' => $xAxis1 ,'yAxis_setting' => $yAxis1 ,'chartoptions' => serialize($posted_values['chart_options']),'plotoptions' => serialize($posted_values['plot_options']));
...
//						{ 
//						   if($key2 != 'bracket')
//					       $yAxis1 .= $key2.": '".$pval2."'".','."<br/>"; 
//		                }
//	                $yAxis1 .=$brackts1['1'].','."<br/>"; 
//			     }
//		    }
